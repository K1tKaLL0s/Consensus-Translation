...
def test_mode_policies_encode_three_phase3_operating_modes():
    ...
    pretraining = policy_for_mode(
        AgentMode.PRETRAINING, api_enabled=False, budget_limit=0.0
    )
    ...
    assert pretraining.max_rounds == 1
    assert pretraining.human_gate_required is True
    assert pretraining.validation_required is True
    assert pretraining.api_enabled is False
